...
```
Some rare setups (e.g., installation in user-level folders of dated server distributions) cause ```pipits_process``` to fail while converting to biom format. The issue is solved by  updating the fresh installation from within the environment: ```conda update pipits```.
